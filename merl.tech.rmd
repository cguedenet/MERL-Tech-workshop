---
title: "2018 MERL Tech DC workshop: Up your Data Game with R"
output:
  html_document:
...
```{r cleaning, message=FALSE, warning=FALSE, echo=TRUE}
...
```{r summary, warning=FALSE, message=FALSE ,echo=TRUE, results = 'asis'}
library(formattable)
...
## add new stats  
```{r summary2,warning=FALSE, message=FALSE ,echo=TRUE, results = 'asis'}
df %>% summarise(
...
```{r grouping, warning=FALSE, message=FALSE ,echo=TRUE, results = 'asis'}
df %>%
  group_by(Gender) %>%
  summarise(
    mean = mean(Age, na.rm = TRUE),
...
```{r warning=FALSE, message=FALSE ,echo=TRUE, eval=TRUE, results = 'asis'}
df %>% ggplot(aes(fct_infreq(EmploymentField))) + #fct_infreq is an easy way to order factors using the forcats package
  geom_bar() +
...
Compare only males and females  
